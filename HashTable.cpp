...
unsigned int Rolling(const char* word) {
    ...
    for(int i = 0; word[i]; i++, hash += (((word[i] >> 7) & 1) | (word[i] << 1)) );
...
unsigned int VeryBadHash(const char* word) {
...
int main() {
    ...
    Table[6].HashFunc = GavHash;
    Table[6].HashName = "Gav";
    //add hash here
